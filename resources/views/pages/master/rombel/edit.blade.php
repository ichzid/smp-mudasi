...
@section('content')
    <div class="space-y-6">
        <!-- Breadcrumb -->
        <x-common.page-breadcrumb pageTitle="Edit Data Rombel" pageSubtitle="Perbarui informasi rombongan belajar." />
            ...
            <form action="{{ route('master.rombel.update', $rombel->id) }}" method="POST" class="space-y-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <div x-data="{ isOptionSelected: true }" class="relative z-20 bg-transparent">
                            <select id="tingkat" name="tingkat" required
                                ...
                                <option value="7" class="text-gray-700 dark:bg-gray-900 dark:text-gray-400" {{ old('tingkat', $rombel->tingkat) == '7' ? 'selected' : '' }}>Kelas 7</option>
                                <option value="8" class="text-gray-700 dark:bg-gray-900 dark:text-gray-400" {{ old('tingkat', $rombel->tingkat) == '8' ? 'selected' : '' }}>Kelas 8</option>
                                <option value="9" class="text-gray-700 dark:bg-gray-900 dark:text-gray-400" {{ old('tingkat', $rombel->tingkat) == '9' ? 'selected' : '' }}>Kelas 9</option>
                            </select>
                            <span class="pointer-events-none absolute top-1/2 right-4 z-30 -translate-y-1/2 text-gray-500 dark:text-gray-400">
                                <svg class="stroke-current" width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
                                    <path d="M4.79175 7.396L10.0001 12.6043L15.2084 7.396" stroke="" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round" />
                    ...
                    <div>
                        ...
                        <input type="text" id="nama_rombel" name="nama_rombel" value="{{ old('nama_rombel', $rombel->nama_rombel) }}" required placeholder="Contoh: 7A, 8B, 9-Alpha"
                        ...
                        <div x-data="{ isOptionSelected: true }" class="relative z-20 bg-transparent">
                            <select id="wali_kelas_id" name="wali_kelas_id" required
                                ...
                                @foreach($gurus as $guru)
                                    <option value="{{ $guru->id }}" class="text-gray-700 dark:bg-gray-900 dark:text-gray-400" {{ old('wali_kelas_id', $rombel->wali_kelas_id) == $guru->id ? 'selected' : '' }}>
