...
@section('content')
    <div class="space-y-6">
        ...
        <x-common.page-breadcrumb 
            ...
            pageSubtitle="Tambahkan kelas baru ke dalam sistem."
        />
        ...
        <div class="rounded-2xl border border-gray-200 bg-white p-5 sm:p-8 shadow-sm dark:border-gray-800 dark:bg-white/[0.03]">
            <form action="{{ route('master.rombel.store') }}" method="POST" class="space-y-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div x-data="{ isOptionSelected: false }" class="relative z-20 bg-transparent">
                            <select id="tahun_ajaran_id" name="tahun_ajaran_id" required
                                ...
                                @foreach($tahun_ajarans as $ta)
                                    <option value="{{ $ta->id }}" class="text-gray-700 dark:bg-gray-900 dark:text-gray-400" {{ old('tahun_ajaran_id') == $ta->id || ($ta->is_aktif && !old('tahun_ajaran_id')) ? 'selected' : '' }}>
                    ...
                    <!-- Tingkat -->
                    <div>
                        <label for="tingkat" class="mb-1.5 block text-sm font-medium text-gray-700 dark:text-gray-400">Tingkat <span class="text-red-500">*</span></label>
                        <div x-data="{ isOptionSelected: false }" class="relative z-20 bg-transparent">
                            <select id="tingkat" name="tingkat" required
                                class="dark:bg-dark-900 shadow-theme-xs focus:border-brand-300 focus:ring-brand-500/10 dark:focus:border-brand-800 h-11 w-full appearance-none rounded-lg border border-gray-300 bg-transparent bg-none px-4 py-2.5 pr-11 text-sm text-gray-800 placeholder:text-gray-400 focus:ring-3 focus:outline-hidden dark:border-gray-700 dark:bg-gray-900 dark:text-white/90 dark:placeholder:text-white/30"
                                :class="isOptionSelected && 'text-gray-800 dark:text-white/90'" @change="isOptionSelected = true">
                                <option value="" class="text-gray-700 dark:bg-gray-900 dark:text-gray-400">Pilih Tingkat</option>
                                <option value="7" class="text-gray-700 dark:bg-gray-900 dark:text-gray-400" {{ old('tingkat') == '7' ? 'selected' : '' }}>Tingkat 7</option>
                                <option value="8" class="text-gray-700 dark:bg-gray-900 dark:text-gray-400" {{ old('tingkat') == '8' ? 'selected' : '' }}>Tingkat 8</option>
                                <option value="9" class="text-gray-700 dark:bg-gray-900 dark:text-gray-400" {{ old('tingkat') == '9' ? 'selected' : '' }}>Tingkat 9</option>
                            </select>
                            <span class="pointer-events-none absolute top-1/2 right-4 z-30 -translate-y-1/2 text-gray-500 dark:text-gray-400">
                                <svg class="stroke-current" width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
                                    <path d="M4.79175 7.396L10.0001 12.6043L15.2084 7.396" stroke="" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round" />
                    ...
                    <!-- Nama Kelas -->
                    <div>
                        <label for="nama" class="mb-1.5 block text-sm font-medium text-gray-700 dark:text-gray-400">Nama Kelas <span class="text-red-500">*</span></label>
                        <input type="text" id="nama" name="nama" value="{{ old('nama') }}" required maxlength="50" placeholder="Contoh: 7"
                        ...
                        <div x-data="{ isOptionSelected: false }" class="relative z-20 bg-transparent">
                            <select id="wali_guru_id" name="wali_guru_id" required
                                ...
                                @foreach($gurus as $guru)
                                    <option value="{{ $guru->id }}" class="text-gray-700 dark:bg-gray-900 dark:text-gray-400" {{ old('wali_guru_id') == $guru->id ? 'selected' : '' }}>
