            ...
            <form action="{{ route('master.siswa.store') }}" method="POST" enctype="multipart/form-data" class="space-y-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div x-data="{ isOptionSelected: true }" class="relative z-20 bg-transparent">
                            <select id="status" name="status" required
                                ...
                                @foreach(['aktif' => 'Aktif', 'lulus' => 'Lulus', 'pindah' => 'Pindah', 'keluar' => 'Keluar'] as $value => $label)
                                    <option value="{{ $value }}" class="text-gray-700 dark:bg-gray-900 dark:text-gray-400" {{ old('status', 'aktif') === $value ? 'selected' : '' }}>{{ $label }}</option>
                                @endforeach
                            </select>
                    ...
                    <div>
                        ...
                        <input type="text" id="no_hp_wali" name="no_hp_wali" value="{{ old('no_hp_wali') }}"
                            class="dark:bg-dark-900 shadow-theme-xs focus:border-brand-300 focus:ring-brand-500/10 dark:focus:border-brand-800 h-11 w-full rounded-lg border border-gray-300 bg-transparent px-4 py-2.5 text-sm text-gray-800 placeholder:text-gray-400 focus:ring-3 focus:outline-hidden dark:border-gray-700 dark:bg-gray-900 dark:text-white/90 dark:placeholder:text-white/30">
                        @error('no_hp_wali')
                            <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
                        @enderror
