            ...
            <div>
                ...
                <select name="rombel_id" class="h-11 w-full rounded-lg border border-gray-300 bg-transparent px-3 text-sm dark:border-gray-700 dark:text-white">
                    ...
                    @foreach($rombels as $item)
                        <option value="{{ $item->id }}" @selected($rombel?->id === $item->id)>{{ $item->tingkat }} - {{ $item->nama }}</option>
                    @endforeach
                </select>
            </div>
            <div>
                <label class="mb-1.5 block text-sm font-medium text-gray-700 dark:text-gray-400">{{ $tab === 'harian' ? 'Tanggal' : 'Bulan' }}</label>
                <input type="{{ $tab === 'harian' ? 'date' : 'month' }}" name="{{ $tab === 'harian' ? 'tanggal' : 'bulan' }}" value="{{ $tab === 'harian' ? $tanggal : $bulan }}" class="h-11 w-full rounded-lg border border-gray-300 bg-transparent px-3 text-sm dark:border-gray-700 dark:text-white">
            </div>
    ...
    @if($rombel)
        ...
        <div class="rounded-2xl border border-gray-200 bg-white shadow-sm dark:border-gray-800 dark:bg-white/[0.03]">
            <div class="flex flex-col gap-3 border-b border-gray-200 p-5 sm:flex-row sm:items-center sm:justify-between dark:border-gray-800">
                <div><h3 class="font-semibold text-gray-900 dark:text-white">{{ $rombel->tingkat }} - {{ $rombel->nama }}</h3><p class="text-sm text-gray-500">{{ $tab === 'harian' ? \Carbon\Carbon::parse($tanggal)->translatedFormat('d F Y') : \Carbon\Carbon::createFromFormat('Y-m', $bulan)->translatedFormat('F Y') }}</p></div>
            ...
            <div class="overflow-x-auto">
                <table class="w-full text-left text-sm">
                    ...
                    <tbody class="divide-y divide-gray-200 dark:divide-gray-800">
                        @forelse($rows as $row)<tr class="text-gray-700 dark:text-gray-300">@if($tab === 'bulanan')<td class="px-5 py-4">{{ $row->nis }}</td><td class="px-5 py-4 font-medium">{{ $row->nama_lengkap }}</td>@else<td class="px-5 py-4 font-medium">{{ $row->tingkat }} - {{ $row->nama }}</td>@endif @foreach($statuses as $status)<td class="px-4 py-4 text-center">{{ $row->{$status} }}</td>@endforeach<td class="px-4 py-4 text-center font-semibold">{{ $row->total }}</td></tr>@empty<tr><td colspan="9" class="px-5 py-12 text-center text-gray-500">Belum ada data presensi untuk filter ini.</td></tr>@endforelse
                    </tbody>
                </table>
            </div>
        </div>
