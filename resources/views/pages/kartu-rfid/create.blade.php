...
@section('content')
    <div class="space-y-6">
        <!-- Breadcrumb -->
        <x-common.page-breadcrumb pageTitle="Tambah Kartu RFID" pageSubtitle="Hubungkan kartu RFID baru untuk siswa." />

        <div class="rounded-2xl border border-gray-200 bg-white p-5 sm:p-8 shadow-sm dark:border-gray-800 dark:bg-white/[0.03]">
            <form action="{{ route('kartu-rfid.store') }}" method="POST" class="space-y-6">
                @csrf
                
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <!-- Siswa -->
                    <div class="md:col-span-2">
                        <label for="siswa_id" class="mb-1.5 block text-sm font-medium text-gray-700 dark:text-gray-400">Pilih Siswa <span class="text-red-500">*</span></label>
                        <div x-data="{ isOptionSelected: false }" class="relative z-20 bg-transparent">
                            <select id="siswa_id" name="siswa_id" required
                                ...
                                <option value="" class="text-gray-700 dark:bg-gray-900 dark:text-gray-400">-- Pilih Siswa yang belum memiliki kartu --</option>
                                @foreach($siswas as $siswa)
                                    <option value="{{ $siswa->id }}" class="text-gray-700 dark:bg-gray-900 dark:text-gray-400" {{ old('siswa_id') == $siswa->id ? 'selected' : '' }}>
                        ...
                        <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">Hanya menampilkan siswa yang belum dihubungkan dengan kartu RFID.</p>
                        @error('siswa_id')
                    ...
                    <!-- Kode UID RFID -->
                    <div>
                        <label for="kode_uid" class="mb-1.5 block text-sm font-medium text-gray-700 dark:text-gray-400">Kode UID (Scan Kartu) <span class="text-red-500">*</span></label>
                        <div class="relative">
                            ...
                            <input type="text" id="kode_uid" name="kode_uid" value="{{ old('kode_uid') }}" required autofocus placeholder="Tap kartu RFID ke reader..."
                    ...
                    <div>
                        ...
                        <div x-data="{ isOptionSelected: true }" class="relative z-20 bg-transparent">
                            <select id="status" name="status" required
                                ...
                                <option value="aktif" class="text-gray-700 dark:bg-gray-900 dark:text-gray-400" {{ old('status', 'aktif') == 'aktif' ? 'selected' : '' }}>Aktif</option>
                                <option value="nonaktif" class="text-gray-700 dark:bg-gray-900 dark:text-gray-400" {{ old('status') == 'nonaktif' ? 'selected' : '' }}>Non-aktif</option>
                                <option value="hilang" class="text-gray-700 dark:bg-gray-900 dark:text-gray-400" {{ old('status') == 'hilang' ? 'selected' : '' }}>Hilang</option>
                            </select>
                            <span class="pointer-events-none absolute top-1/2 right-4 z-30 -translate-y-1/2 text-gray-500 dark:text-gray-400">
                                <svg class="stroke-current" width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
                                    <path d="M4.79175 7.396L10.0001 12.6043L15.2084 7.396" stroke="" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round" />
