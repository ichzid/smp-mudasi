...
<div {{ $attributes->only(['class', 'x-on:date-change', '@date-change']) }} x-data="{
    ...
    <div class="relative custom-datepicker">
        <input
            ...
            class="h-11 w-full cursor-pointer rounded-lg border appearance-none px-4 py-2.5 text-sm shadow-theme-xs placeholder:text-gray-400 focus:outline-hidden focus:ring-3 dark:bg-gray-900 dark:text-white/90 dark:placeholder:text-white/30 bg-transparent text-gray-800 border-gray-300 focus:border-brand-300 focus:ring-brand-500/20 dark:border-gray-700 dark:focus:border-brand-800"
            autocomplete="off"
            data-date-format="{{ $dateFormat }}"
            {{ $attributes->except(['class', 'x-on:date-change', '@date-change']) }}
        />
        <button type="button" class="absolute top-1/2 right-3 z-20 -translate-y-1/2 cursor-pointer text-gray-500 dark:text-gray-400" @click="$refs.dateInput._flatpickr?.open()" aria-label="Buka kalender">
