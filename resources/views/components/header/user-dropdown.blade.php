<div class="relative" x-data="{
    ...
    <button
        ...
        type="button"
    >
        <span class="mr-3 flex h-11 w-11 items-center justify-center rounded-full bg-brand-50 font-semibold text-brand-600 dark:bg-brand-500/10 dark:text-brand-400">
    ...
    >
        ...
        <ul class="flex flex-col gap-1 pt-4 pb-3 border-b border-gray-200 dark:border-gray-800">
            @php
                $menuItems = [
                    [
                        'text' => 'Profil Saya',
                        'icon' => \App\Helpers\MenuHelper::getIconSvg('profile'),
                        'path' => route('profile'),
                    ],
                ];
            @endphp
        ...
        <form method="POST" action="{{ route('logout') }}">
            ...
            >
                <span class="text-gray-500 group-hover:text-gray-700 dark:group-hover:text-gray-300">
                    {!! \App\Helpers\MenuHelper::getIconSvg('logout') !!}
                </span>
                Keluar
            </button>
        </form>
